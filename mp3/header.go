...
// extracted from https://github.com/badgerodon/mp3/blob/master/header.go
...
// парсим информацию из первого доступного MPEG фрейма
func GetFirstFrame(r io.Reader) (*FrameHeader, error) {
	var bs [4]byte

	for bs[0] != 0xFF || (bs[1]&0xE0) != 0xE0 {
		if _, err := io.ReadFull(r, bs[0:2]); err != nil {
			if err == io.ErrUnexpectedEOF {
				return nil, errors.New("no MPEG frame found")
			}
			return nil, err
		}
	}

	if _, err := io.ReadFull(r, bs[2:4]); err != nil {
		return nil, err
	}

	frame := &FrameHeader{}
	if err := frame.Parse(bs[0:4]); err != nil {
		return nil, err
	}
